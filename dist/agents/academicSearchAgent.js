...
const createBasicAcademicSearchRetrieverChain = (llm) => {
    return runnables_1.RunnableSequence.from([
        prompts_1.PromptTemplate.fromTemplate(basicAcademicSearchRetrieverPrompt),
        llm,
        strParser,
...
const basicAcademicSearch = (query, history, llm, embeddings) => {
    ...
    try {
        ...
        logger_1.default.error(`Error in academic search: ${err}`);
    }
    return emitter;
};
const handleAcademicSearch = (message, history, llm, embeddings) => {
    const emitter = basicAcademicSearch(message, history, llm, embeddings);
    return emitter;
};
exports.default = handleAcademicSearch;
