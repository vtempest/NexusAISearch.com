...
export default function convertHTMLToBasicHTML(html, options) {
  options = options || { images: 1, 
    links: 1, formatting: 1, 
    absoluteURLs: 1,
    url: 0 };
  ...
  if (options.images) allowedTags.push("img");
  ...
  var basicHtml =  convertHTMLToTokens(html)
    ...
    .map(el => {
      ...
      if(options.absoluteURLs && urlValue){
        if(urlValue.startsWith("//"))
          urlValue = "https:"+urlValue;

         if(urlValue[0]=="#")
          urlValue = options.url+urlValue;
         try{
         if(urlValue[0]=="/")
          urlValue = (new URL(options.url)).origin+urlValue;
        }catch(e){ }
