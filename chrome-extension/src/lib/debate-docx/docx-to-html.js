...
*/
const documentToTokens = async (docxPath, options) => {
  if (!docxPath) return;

  //load file or download url into memory buffer
  var buffer = Buffer.isBuffer(docxPath) ? docxPath :
...
exports.documentToTokens = documentToTokens;
/** 
  1 - open document.xml
  2 - tokenize xml
  3 - reconstruct cleaned html
*/
const documentToMarkup = async (filepath) => {
  const docTokens = await exports.documentToTokens(filepath);
  const markup = tokens.tokensToMarkup(docTokens);
  return markup;
};
exports.documentToMarkup = documentToMarkup;

const createStyleParser = async (styleXML) => {
  const parsedStyles = {};
  let styleName = "";
    ...
    var parser = new Parser(
      {
        onopentag(name, attributes) {
          ...
          if (styleName) {
            ...
            else if (name === "w:highlight" || styleName.toLowerCase().includes('highli') ) 
               styles.mark = true;
            else if (name === "w:b")
...
exports.createStyleParser = createStyleParser;
 ...
 */
const createTokenizer = async (docXML, styleData) => {
  const blocks = [];
  let block;
  let token;
  return await new Promise((resolve, reject) => {
