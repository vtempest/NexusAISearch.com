...
const parseCard = (doc, anchor = 0, idx) => {
  ...
  function htmlToRanges(html) {
    var text = "" , marked = "", underlined = "", isUnderline = 0, isHighlight = 0;
    var htmltags = html
      .split("<")
      .map((tagNode) => {
        ...
        if (isHighlight && content)
          marked += content.trim() + "…" || "" ;
        if (isUnderline && content)
          underlined += content.trim()+ "…"  || "" ;
