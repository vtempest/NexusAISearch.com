...
function getDate() {
  ...
  for (let el of time) {
    if (el.getAttribute('datetime') && (el.getAttribute('datetime'))) {
...
export function extractDate(document) {
  ...
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("Date");
  } //Try Date
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("created");
  } //Try created
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("dat");
  } //Try dat
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("DC.date");
  } //Try DC.date
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("dc.date");
  } //Try dc.date
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("DC.date.issued");
  } //Try DC.date.issued
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("dc.date.issued");
  } //Try dc.date.issued
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("dcterms.created");
  } //Try dcterms.created
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("DCterms.created");
  } //Try DCterms.created
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("dcterms.modified");
  } //Try dcterms.modified
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("DCterms.modified");
  } //Try DCterms.modified
  if (arrDates.length <= 0) {
    arrDates = document.getElementsByName("sailthru.date");
  } //Try sailthru.date

  //If anything found, assign it to Date
  if (arrDates.length > 0) {
    date = arrDates[0].content;
  } else {
  ...
  date = Date.parse(date).toISOString().split("T")[0];
