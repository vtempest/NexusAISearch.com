...
function upgrade_0(saved: unknown): unknown {
	...
	for (let i = 0; i < flows.length; i++) {
		...
		for (let j = 0; j < flow.children.length; j++) {
			...
			upgrade_0_addBoxesRec(nodes, flowId, flowId, box, j);
		}
	}
...
function upgrade_0_addBoxesRec(
	nodes: Nodes,
	flowId: FlowId,
	...
	for (let i = 0; i < box.children.length; i++) {
		upgrade_0_addBoxesRec(nodes, flowId, id, box.children[i], i);
	}
}
